...
const SessionHistory: React.FC = () => {
  ...
  const createDemoSession = async () => {
    try {
      ...
      toast.error('Could not create demo session');
    }
  };
  ...
  return (
    <HistoryContainer>
      ...
      {sessions.length === 0 ? (
        <EmptyState>
          No sessions found yet. Run a QKD simulation or create one demo session.
          <div>
            <DemoButton onClick={createDemoSession}>Create Demo Session</DemoButton>
          </div>
        </EmptyState>
      ) : (
        <SessionsGrid>
